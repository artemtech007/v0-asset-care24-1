-- =====================================================
-- AssetCare24 MVP Database Schema v2.3
-- Separate tables for clients and masters
-- Added verification fields for WhatsApp, email, and phone
...
CREATE TABLE public.clients (
    ...
    first_name text,                        -- Optional - can be filled later
    last_name text,                         -- Optional - can be filled later
    ...
    cod_value varchar(20),                  -- COD (Cash on Delivery) идентификатор
...
CREATE TABLE public.masters (
    ...
    admin_comment text,
    wa_norm varchar(32),                    -- Нормализованный WhatsApp номер (32 символа)
    wa_verified boolean DEFAULT false,      -- Верификация WhatsApp номера
    email_verified boolean DEFAULT false,   -- Верификация email адреса
    phone_verified boolean DEFAULT false,   -- Верификация номера телефона
    code_source text,                       -- Источник регистрации (из кода)
...
-- =====================================================
-- 11. PROCESSED MESSAGES TABLE (Webhook deduplication)
-- =====================================================
...
CREATE TABLE public.master_settings (
    ...
    -- Working schedule and service area
    service_area text,
    working_hours jsonb,
...
-- Sample master
INSERT INTO public.masters (id, phone, whatsapp_id, first_name, last_name, status, specializations, wa_norm)
VALUES ('mid_wa_491510416556', '+491510416556', 'whatsapp_master_456', 'Алексей', 'Мастеров', 'active', ARRAY['elektrik', 'sanitär'], '+491510416556');
