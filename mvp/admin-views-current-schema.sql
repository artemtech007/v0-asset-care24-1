-- =====================================================
-- Admin Views for Current Database Schema
-- =====================================================
-- Views адаптированные под текущую схему БД (без rating, completed_jobs)
...
SELECT
    ...
    -- Рейтинг отсутствует в текущей схеме
    NULL as rating
FROM clients c
...
SELECT
    ...
    email_verified,
    phone_verified,
    -- Подсчет завершенных заказов для мастеров (поле completed_jobs отсутствует)
...
SELECT
    ...
    -- Поля category и subcategory могут отсутствовать в clients
    NULL as client_category,
    NULL as client_subcategory,

    -- Информация о адресе (таблица client_addresses может отсутствовать)
...
SELECT
    ...
    -- Доход (поле price может отсутствовать)
    0 as total_revenue;
