 ...
 * Изменение статуса пользователя (клиент или мастер)
 *
 * Тело запроса:
 * {
...
export async function POST(
  ...
  try {
    ...
    if (!newStatus || !['active', 'inactive', 'blocked'].includes(newStatus)) {
      return NextResponse.json(
        { data: null, error: 'Invalid status. Must be "active", "inactive", or "blocked"' },
        { status: 400 }
      )
    }

    // Маппинг статусов UI в БД
    const dbStatusMap = {
      'active': 'active',
      'inactive': 'inactive',
      'blocked': 'blocked'
    ...
    const dbStatus = dbStatusMap[newStatus as keyof typeof dbStatusMap]
    ...
    console.error('API Error /api/admin/users/[id]/status:', error)
